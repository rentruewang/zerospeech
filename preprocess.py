...
def preprocess(
    ...
    print(
        "[Processor] - making stage 1 training samples with segment length = ", seg_len
    )
    make_samples(
        dataset_path,
        index_path,
...
def make_dataset(grps, root_dir, make_test=False):
    ...
    for filename in filenames:
        ...
        speaker_id, segment_id = (
            filename.strip().split("/")[-1].strip(".wav").split("_")
        )
    ...
    for speaker_id, filenames in filename_groups.items():
        for filename in filenames:
            ...
            grp.create_dataset(
                f"{speaker_id}/{segment_id}/mel", data=mel_spec, dtype=np.float32
            )
            grp.create_dataset(
                f"{speaker_id}/{segment_id}/lin", data=lin_spec, dtype=np.float32
            )
        print()
    print()
...
class Sampler(object):
    def __init__(
        self, h5_path, dset="train", seg_len=64, speaker2id_path="", make_object="all"
    ):
        ...
        if make_object == "all":
            ...
            self.speaker_used = [
                s
                for s in sorted(list(self.f_h5[dset].keys()))
                if s not in ["V001", "V002"]
            ]
        ...
        self.speaker2utts = {
            speaker: sorted(list(self.f_h5[f"{dset}/{speaker}"].keys()))
            for speaker in self.speaker_used
        }
    ...
    def rm_too_short_utt(self, limit=None):
        ...
        for speaker_id in self.speaker_used:
            for utt_id in self.speaker2utts[speaker_id]:
                if (
                    self.f_h5[f"{self.dset}/{speaker_id}/{utt_id}/lin"].shape[0]
                    <= limit
                ):
        ...
        print(
            "[Sampler] - %i too short utterences out of a total of %i are removed."
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-source", "--source_path", type=str, default="./data/english/train/unit/"
    )
    parser.add_argument(
        "-target", "--target_path", type=str, default="./data/english/train/voice/"
    )
    parser.add_argument(
        "-test", "--test_path", type=str, default="./data/english/test/"
    )
    parser.add_argument(
        "-dataset", "--dataset_path", type=str, default="./data/dataset.hdf5"
    )
    parser.add_argument(
        "-indexp", "--index_path", type=str, default="./data/index.json"
    )
    parser.add_argument(
        "-indexs", "--index_source_path", type=str, default="./data/index_source.json"
    )
    parser.add_argument(
        "indext", "--index_target_path", type=str, default="./data/index_target.json"
    )
    parser.add_argument(
        "-s2i", "--speaker2id_path", type=str, default="./data/speaker2id.json"
    )

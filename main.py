...
if __name__ == '__main__':
    ...
    parser.add_argument('--preprocess', default=False,
                        action='store_true', help='preprocess the zerospeech dataset')
    parser.add_argument('--train', default=False, action='store_true',
                        help='start stage 1 and stage 2 training')
    parser.add_argument('--test', default=False, action='store_true',
                        help='test the trained model on all testing files')
    parser.add_argument('--test_single', default=False, action='store_true',
    ...
    static_setting.add_argument('--targeted_G', type=bool, default=bool(
        1), help='G can only convert to target speakers and not all speakers')
    static_setting.add_argument('--one_hot', type=bool, default=bool(
        0), help='Set the encoder to encode to symbolic discrete one-hot vectors')
    static_setting.add_argument('--binary_output', type=bool, default=bool(
        1), help='Set the encoder to produce binary 1/0 output vectors')
    static_setting.add_argument(
        '--binary_ver', type=int, default=0, help='Set the binary type of the encoder output')
    static_setting.add_argument('--enc_only', type=bool, default=bool(
        1), help='whether to predict only with stage 1 audoencoder')
    static_setting.add_argument('--s_speaker', type=str, default='S015',
                                help='for the --test_single mode, set voice convergence source speaker')
    static_setting.add_argument('--t_speaker', type=str, default='V001',
    ...
    data_path.add_argument('--source_path', type=str,
                           default='./data/english/train/unit/', help='the zerospeech train unit dataset')
    data_path.add_argument('--target_path', type=str, default='./data/english/train/voice/',
                           help='the zerospeech train voice dataset')
    data_path.add_argument('--test_path', type=str,
                           default='./data/english/test/', help='the zerospeech test dataset')
    data_path.add_argument('--dataset_path', type=str, default='./data/dataset.hdf5',
                           help='the processed train dataset (unit + voice)')
    data_path.add_argument('--index_path', type=str, default='./data/index.json',
                           help='sample training segments from the train dataset, for stage 1 training')
    data_path.add_argument('--index_source_path', type=str, default='./data/index_source.json',
                           help='sample training source segments from the train dataset, for stage 2 training')
    data_path.add_argument('--index_target_path', type=str, default='./data/index_target.json',
                           help='sample training target segments from the train dataset, for stage 2 training')
    data_path.add_argument('--speaker2id_path', type=str,
    ...
    model_path.add_argument('--model_name', type=str,
                            default='model.pth', help='base model name for training')
    model_path.add_argument('--load_train_model_name', type=str, default='model.pth-s1-100000',
                            help='the model to restore for training, the command --load_model will load this model')
    model_path.add_argument('--load_test_model_name', type=str, default='model.pth-s2-150000',
    ...
    if args.train:
        ...
        sourceset = Dataset(args.dataset_path,
                            args.index_source_path, seg_len=hps.seg_len)
        targetset = Dataset(args.dataset_path,
                            args.index_target_path, seg_len=hps.seg_len)
        ...
        trainer = Trainer(hps, data_loader, args.targeted_G,
                          args.one_hot, args.binary_output, args.binary_ver)
        if args.load_model:
            trainer.load_model(os.path.join(
                args.ckpt_dir, args.load_train_model_name), model_all=False)
    ...
    if args.test or args.test_single:
        ...
        trainer = get_trainer(args.hps_path, model_path, args.targeted_G,
                              args.one_hot, args.binary_output, args.binary_ver)
